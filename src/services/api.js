...
const API_BASE_URL = process.env.REACT_APP_API_URL || 'http://192.168.1.136:8080';
...
export const productAPI = {
  ...
  getTopSelling: async () => {
    try {
      ...
      const sorted = [...mockProducts].sort((a, b) => (b.sales || 0) - (a.sales || 0));
      return new Promise(resolve => setTimeout(() => resolve(sorted.slice(0, 5)), 300));
    }
  },
  ...
  getLatest: async () => {
    try {
      ...
      const sorted = [...mockProducts].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
      return new Promise(resolve => setTimeout(() => resolve(sorted.slice(0, 5)), 300));
    }
  },
};
...
export const orderAPI = {
  getAll: async () => {
    try {
      ...
      return new Promise(resolve => setTimeout(() => resolve(mockRecentOrders), 500));
    }
  },
  ...
  create: async (userId, orderData) => {
    try {
      ...
      return new Promise(resolve => setTimeout(() => resolve(mockOrder), 800));
    }
  },
...
export const userAPI = {
  // ✅ Real login – errors are thrown and handled in the UI
...
export const analyticsAPI = {
  ...
  getRevenueData: async (period = 'month') => {
    try {
      return await apiClient.get(`${ENDPOINTS.getTotalRevenue}?period=${period}`);
    } catch (error) {
      const mockData = [
...
export const paymentAPI = {
  processPayment: async (paymentData) => {
    try {
      ...
      return new Promise(resolve => setTimeout(() => resolve(mockPaymentResponse), 1500));
    }
  },
  ...
  savePaymentMethod: async (methodData) => {
    try {
      return await apiClient.post(ENDPOINTS.addPaymentMethod(methodData.userId), methodData);
    } catch (error) {
